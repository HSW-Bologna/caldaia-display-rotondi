...
static void update_page(model_t *model, struct page_data *pdata) {
    float pressure = ((float)(model->run.pressure_centibar)) / 100.;
    uint16_t offset   = model->config.pressure_offset_adc;

    lv_label_set_text_fmt(pdata->lbl_adc, "Pressione: %2.2f Bar [%04i-%04i]\nFasi: r1=%04i, s=%04i, t=%04i",
                          pressure, model->run.pressure_adc, offset, model->run.adc_r1, model->run.adc_s,
                          model->run.adc_t);
